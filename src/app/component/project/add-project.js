export function generateProject(project) {
  ...
  let infoCardModal = createDomNode(imgCardModal, 'div', 'project-info');
  body.append(infoCardModal);

  let infoList = createDomNode(infoList, 'ul', 'info-list', 'anim-items');
  ...
  let infoParagraph = createDomNode(
    infoParagraph,
    'h5',
    'info-paragraph',
    'anim-items'
  ...
  let infoDemo = createDomNode(infoDemo, 'a', 'button_primary', 'anim-items');
  infoDemo.innerHTML = `LIVE DEMO`;
  infoDemo.href = project.demo;
  buttonsModal.append(infoDemo);

  let infoCode = createDomNode(infoCode, 'a', 'button_primary', 'anim-items');
